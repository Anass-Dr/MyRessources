...
<?php 
  ...
  $categoryTabFlds = $db->query("DESCRIBE categories");
  $allFields = $categoryTabFlds->fetch_all();
  $fieldsNum = count($allFields);

  #- Get all users from db :
  $categoryTable = $db->query("SELECT * FROM categories");
  $categoryRows = $categoryTable->fetch_all();
?>
<!DOCTYPE html>
<html lang="en">
  <head>
  ...
  <body>
    ...
    <aside>
      ...
      <main class="flex-grow-1 d-flex flex-column px-4 py-3">
        ...
        <span class="row_count"><?php echo count($categoryRows) ?> Users Found</span>
        ...
        <ul class="d-flex align-items-center p-0 mt-4 gap-4 filter_ul">
          ...
          <table class="w-100 mt-4">
            ...
            <tbody>
              <?php for ($i = 0; $i < count($categoryRows); $i++) : ?>
                    ...
                    <tr class="active_tr<?php if ($i > 4) echo ' hidden' ?>">
                      <td><?php echo $categoryRows[$i][0] ?></td>
                      <td><?php echo $categoryRows[$i][1] ?></td>
                      <td>
              ...
              <div class="modal-content">
                ...
                <form action="./category/update.php" method="POST">
                  <div class="modal-body">
                    <div class="mb-3">
                      <label for="user_name" class="form-label">Category Name</label>
          ...
          >
            <li id="prev"><</li>
            <?php for ($i = 0; $i < count($categoryRows) / 5; $i++) : ?>
