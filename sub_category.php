...
<?php 
  ...
  $subcategoryTabFlds = $db->query("DESCRIBE subcategories");
  $allFields = $subcategoryTabFlds->fetch_all();
  $fieldsNum = count($allFields);

  #- Get all users from db :
  $subcategoryTable = $db->query("SELECT * FROM subcategories");
  $subcategoryRows = $subcategoryTable->fetch_all();
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <aside>
      ...
      <main class="flex-grow-1 d-flex flex-column px-4 py-3">
        <header class="d-flex justify-content-between">
          <h2 class="fs-2">SubCategory</h2>
          <div class="d-flex align-items-center gap-3">
            <i class="fa-solid fa-bell"></i><i class="fa-solid fa-gear"></i>
            <div class="profile_img"></div>
          </div>
        </header>
        <span class="row_count"><?php echo count($subcategoryRows) ?> SubCategories Found</span>
        ...
        <ul class="d-flex align-items-center p-0 mt-4 gap-4 filter_ul">
          ...
          <table class="w-100 mt-4">
            ...
            <tbody>
            <?php for ($i = 0; $i < count($subcategoryRows); $i++) : ?>
                    ...
                    <tr class="active_tr<?php if ($i > 4) echo ' hidden' ?>">
                      <td><?php echo $subcategoryRows[$i][0] ?></td>
                      <td><?php echo $subcategoryRows[$i][1] ?></td>
                      <td><?php echo $subcategoryRows[$i][2] ?></td>
                      <td>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="updateModalLabel">Update SubCategory</h1>
                ...
                <form action="./subcategory/update.php" method="POST">
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      ...
                      <?php
                        $subTable = $db->query('SELECT * FROM categories');
